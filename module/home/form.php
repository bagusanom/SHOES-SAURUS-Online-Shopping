		...
		<ul>
			<?php 
				if($kategori_id)
					...
					{
						// if ($query = mysqli_query($koneksi, "SELECT barang.*, kategori.kategori FROM barang JOIN kategori ON barang.kategori_id=kategori.kategori_id ORDER BY barang_id"))
						// 	{
						// 			echo "<p><b>$row[kategori]</b><p/>";
						// 	}
						$query = mysqli_query($koneksi,"SELECT * FROM kategori WHERE kategori_id='$kategori_id'");
							
								$row=mysqli_fetch_assoc($query);
        						echo "<p><h2 style:'text-align: center;'>$row[kategori]</h2><p/>";
        						
						if($query = mysqli_query($koneksi, "SELECT a.*, b.kategori FROM barang AS a INNER JOIN kategori AS b ON a.kategori_id = b.kategori_id 
															WHERE b.kategori_id='$kategori_id' ORDER BY a.barang_id"))
							// SELECT b.kategori FROM barang AS a INNER JOIN kategori AS b ON a.kategori_id = b.kategori_id
